...
if ($_POST) {
    ...
    $password = strip_tags($password);
        ...
        if($res['status'] == 'error'){
            ...
            if($_SESSION["ses_ctipo"]=='I')
            {
                $_SESSION["ses_dtipo"] = 'Inversionista';
            }

            if($_SESSION["ses_ctipo"]=='E')
            {
            ...
            // obtengo permisos
            $objPermisos = $myIns->get_userpermisos($DBcon,$objDatosUsr->id);
            $cont = 1;
            while ($row = $objPermisos->fetchObject()) {

                $_SESSION["ses_p_".$row->idpermiso] = $row->cpermiso;
            }
